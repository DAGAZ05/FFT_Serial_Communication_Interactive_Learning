...
if __name__ == '__main__':
    ...
    # end = 1000
    prjNum = end - start + 1
    source_path = os.path.join(script_dir, 'rawvertexes.txt')
    with open(source_path, 'r') as f:#读取并解析SVG信息
        rawdata = f.readlines()
    ...
    data_path = os.path.join(script_dir, 'datas.txt')
    with open(data_path, "w") as File:#把结果写入硬盘
        for dc in out:
